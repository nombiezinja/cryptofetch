...
const getCurrentDataFromBaseToQuoted = async (baseCurrency, quotedCurrency) => {
  const fsyms = baseCurrency.toUpperCase();
  const tsyms = quotedCurrency.toUpperCase();
  try {
    const response = await fetch(`https://min-api.cryptocompare.com/data/pricemultifull?fsyms=${fsyms}&tsyms=${tsyms}`)
...
const populateHourly = async (name, displayName) => {
  ...
  if (usdJson.data) {
    usdJson.data.forEach((entry) => {
      Hourly.save(usdJson.name, usdJson.displayName, entry).then(() => {
      }).catch((err) => {
        console.log(err);
      });
    });
  ...
  if (btcJson.data) {
    btcJson.data.forEach((entry) => {
      Hourly.updateWithBtcInfo(btcJson.name, entry).then(() => {
      }).catch((err) => {
        console.log(err);
      });
    });
...
const populateDaily = async (name, displayName) => {
  ...
  if (usdJson.data) {
    usdJson.data.forEach((entry) => {
      ...
      Daily.save(usdJson.name, usdJson.displayName, entry).then(() => {
      }).catch((err) => {
        console.log(err);
      });
    });
  ...
  if (btcJson.data) {
    btcJson.data.forEach((entry) => {
      Daily.updateWithBtcInfo(btcJson.name, entry).then(() => {
      }).catch((err) => {
        console.log(err);
      });
    });
...
module.exports = {
  ...
  getCurrentDataFromBaseToQuoted
};