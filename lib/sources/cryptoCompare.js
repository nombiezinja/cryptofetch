...
const constructUrl = (coinId, coinName, currency, period) => {
  return url = `https://min-api.cryptocompare.com/data/histo${period === "daily" ? "day" : "hour"}?fsym=${coinId.toUpperCase()}&tsym=${currency}&limit=${period === 'daily' ? 2000 : 168}&aggregate=1&e=CCCAGG`;
}

const getData = async (coinId, coinName, currency, period) => {
  try {
    const url = await constructUrl(coinId, coinName, currency, period)
...
const populateDaily = async (coinId, coinName) => {

  const btcJson = await getData(coinId, coinName, 'BTC', 'daily');
  const usdJson = await getData(coinId, coinName, 'USD', 'daily');
