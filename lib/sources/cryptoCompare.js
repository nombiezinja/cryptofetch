...
const constructUrl = (name, displayName, currency, period) => {
  return `https://min-api.cryptocompare.com/data/histo${period === 'daily' ? 'day' : 'hour'}?fsym=${name.toUpperCase()}&tsym=${currency}&limit=${period === 'daily' ? 2000 : 168}&aggregate=1&e=CCCAGG`;
};
...
const getCurrentData = async(name) => {
    console.log(`https://min-api.cryptocompare.com/data/price?fsym=${name.toUpperCase()}&tsyms=BTC,USD`)
    const response = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${name.toUpperCase()}&tsyms=BTC,USD`);
    const json = await response.json();
    console.log(json)
    return {
      name: name, 
