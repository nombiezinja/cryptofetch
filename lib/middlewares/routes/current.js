...
module.exports = (paramsMiddleware, DelayedCurrent) => {
  ...
  // router.get('/from/:name/to/:currency', (req, res) => {
  //   cryptoCompare.getCurrentDataInCurrency(req.params.name, req.params.currency).then((result) => {
  //     res.send(result.data['RAW'])
  //   }).catch((error) => {
  //     res.send(error);
  //   })
  // })

  router.get('/from/:name/to/:currency', paramsMiddleware, async (req, res) => {
    DelayedCurrent.getDataFromBaseToQuoted(req.params.name, req.params.currency)
      .then(result => res.send(result).status(200))
      .catch(error => res.send({error: error.message}).status(502));
  });
